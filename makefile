...
train-ranking:
	python sse_train.py --task_type=ranking --data_dir=rawdata-ranking --model_dir=models-ranking  --device=$(device) --learning_rate=$(lr) --alpha=$(alpha) --embedding_size=30 --encoding_size=64 --max_seq_length=60  --batch_size=32 --max_epoc=200 --steps_per_checkpoint=200  --neg_samples=$(neg_samples)
...
train-crosslingual:
	python sse_train.py --task_type=crosslingual --data_dir=rawdata-crosslingual --model_dir=models-crosslingual  --device=$(device) --learning_rate=$(lr) --alpha=$(alpha) --embedding_size=40 --encoding_size=50 --max_seq_length=50  --batch_size=32 --max_epoc=1000 --steps_per_checkpoint=200  --neg_samples=$(neg_samples) --network_mode=shared-encoder
