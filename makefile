...
train-qna:
	python sse_train.py --task_type=qna --data_dir=rawdata-qna --model_dir=models-qna  --batch_size=32 --max_epoc=2000 --steps_per_checkpoint=10 --device=$(device) --learning_rate=$(lr) --vocab_size=8000 --max_seq_length=3000 --alpha=$(alpha) --neg_samples=$(neg_samples)
...
train-ranking:
	python sse_train.py --task_type=ranking --data_dir=rawdata-ranking --model_dir=models-ranking  --device=$(device) --learning_rate=$(lr) --alpha=$(alpha) --embedding_size=30 --encoding_size=50 --max_seq_length=50  --batch_size=128 --max_epoc=300 --steps_per_checkpoint=100  --neg_samples=$(neg_samples)
